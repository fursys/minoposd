...
static float		osd_voltage_factor = 1;
static float		osd_voltage_zero = 0;
//static float 		osd_current_factor = 1;
//static float		osd_current_zero = 0;		
#endif	
...
static uint8_t      osd_armed_prev = 0;             // OP armed info previous status
static uint8_t      osd_time_hour = 0;              // OP GPS time hour info
static uint8_t      osd_time_minute = 0;            // OP GPS tiem minute info
static float 		revo_PS_alt = 0;				// OP Position state object Altitude
#ifdef REVO_ADD_ONS
...
//static uint8_t		oplm_RxGood = 0;				//sergey
#endif
