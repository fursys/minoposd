...
void switchPanels() {
    ...
    if (ch_toggle == 4) {
        if (osd_mode != FLIGHTSTATUS_FLIGHTMODE_POSITIONHOLD) {
...
void panHomeAlt(int first_col, int first_line) {
	...
	osd.printf("%c%5.0f%c", 0xE7, (double)(revo_PS_alt * convert_length), unit_length);
#else
...
void panClimb(int first_col, int first_line) {
    ...
    osd.openPanel();
#ifdef JR_SPECIALS	// I like it more this way
    osd.printf("%c%5.2f%c", 0x16, (double)(osd_climb), 0x88);
#else
    osd.printf("%c%3.2f%c", 0x16, (double)(osd_climb), 0x88);
#endif
    osd.closePanel();
}
...
#ifdef REVO_ADD_ONS_RSSI
    osd.setPanel(first_col, first_line);
    osd.openPanel();
    osd.printf("%4i%c%3i%c", oplm_rssi, 0x8B, oplm_linkquality, 0x8C);
	//osd.printf("%4i%c%3i%c", oplm_rssi, 0x8B, oplm_RxGood, 0x8C);
    osd.closePanel();
#endif
#ifdef ANALOG_RSSI_ON_MINIMOSD
    osd.setPanel(first_col, first_line);
...
void panFlightMode(int first_col, int first_line) {
	...
	else if (osd_mode == FLIGHTSTATUS_FLIGHTMODE_COURSELOCK		) mode_str = "cl ";	
	else if (osd_mode == FLIGHTSTATUS_FLIGHTMODE_POSITIONROAM   ) mode_str = "pra";
	else if (osd_mode == FLIGHTSTATUS_FLIGHTMODE_HOMELEASH		) mode_str = "hl ";
    else if (osd_mode == FLIGHTSTATUS_FLIGHTMODE_RETURNTOBASE   ) mode_str = "rtb";	// RETURNTOBASE
    else if (osd_mode == FLIGHTSTATUS_FLIGHTMODE_LAND           ) mode_str = "lnd";	// LAND
    else if (osd_mode == FLIGHTSTATUS_FLIGHTMODE_PATHPLANNER    ) mode_str = "ppl";	// PATHPLANNER
    else if (osd_mode == FLIGHTSTATUS_FLIGHTMODE_POI            ) mode_str = "poi";	// POI
	else if (osd_mode == FLIGHTSTATUS_FLIGHTMODE_AUTOCRUISE     ) mode_str = "cru";
    else if (osd_mode == FLIGHTSTATUS_FLIGHTMODE_AUTOTAKEOFF     ) mode_str = "ato";
...
void panBatt_A(int first_col, int first_line) {
    ...
    osd.printf("%c%5.2f%c", 0xE2, (double)(osd_vbat_A * osd_voltage_factor), 0x8E);
#else
